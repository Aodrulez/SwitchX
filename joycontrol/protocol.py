...
class ControllerProtocol(BaseProtocol):
    ...
    async def _reply_to_sub_command(self, report):
        ...
        try:
            ...
            #Trying to wake up the device
            elif sub_command == SubCommand.SET_HCI_STATE:
                #print("Entered 0x6 sub command handler")
                await self._command_set_hci_state(sub_command_data)
    ...
    async def _command_set_hci_state(self, sub_command_data):
        #print("inside HCI state handler")
        #print(sub_command_data)
        input_report = InputReport()
        input_report.set_input_report_id(0x21)
        input_report.set_misc()

        input_report.set_ack(0x80)
        input_report.reply_to_subcommand_id(0x6)
